...
/*******************************************************/
/* CUDF solver: leximax_combiner.c                     */
/* Implementation of the leximax combiner              */
...
int leximax_combiner::column_allocation(int first_rank) {
  // Number of criteria
  n = criteria->size();
...
int leximax_combiner::constraint_generation() {

  // Generate the AtLeast constraints
  for (int i = 0; i < n; i++) {
    // add ui <= yj + lmax * lij i.e. yi - uj + lmax * lij >= 0 
    for (int j = 0; j < n; j++) {
...
void leximax_combiner::initialize(CUDFproblem *problem, abstract_solver *solver) {
...
int leximax_combiner::set_variable_range(int first_free_var) { 
...
int leximax_combiner::add_criteria_to_objective(CUDFcoefficient lambda) {
...
int leximax_combiner::add_criteria_to_constraint(CUDFcoefficient lambda) { return 0; }

// Add the required constraints (from the criteria set)
int leximax_combiner::add_constraints() { return constraint_generation(); }

// Compute the range of the combiner/criteria
CUDFcoefficient leximax_combiner::bound_range() {
...
CUDFcoefficient leximax_combiner::upper_bound() {
...
CUDFcoefficient leximax_combiner::lower_bound() {
